...
class KnapsackProblem {
	...
	public String toString() {
		StringBuilder sb = new StringBuilder(this.id+" "+this.weights.length + " " + solution.cost);
...
private void knapsackBF(int index, int state) {
	...
	if(index >= 0) {		
		...
		knapsackBF(index-1, 0);
		knapsackBF(index-1, 1);	
		
	} else {		
		int cost =  sumhelper(mask, this.currentProblem.costs);
		if(cost > this.currentProblem.solution.cost && sumhelper(mask, this.currentProblem.weights) <= this.currentProblem.maxWeight ) {
			this.currentProblem.solution.cost = sumhelper(mask, this.currentProblem.costs);
...
private void knapsackBF(int index, int state, int w, int c) {
	...
	if(index >= 0) {		
		...
		//First I try to compare cost because it is more likely to fail this assert
		if(c > this.currentProblem.solution.cost && w <= this.currentProblem.maxWeight ) {
...
private int knapsack(int i, int W) {
    ...
    if (this.currentProblem.weights[i] > W) {
		this.currentProblem.solution.precense[i] = 0;
        return knapsack(i-1, W);
    } else {
		int left = knapsack(i-1, W);
		int right = knapsack(i-1, W - (this.currentProblem.weights[i])) + (this.currentProblem.costs[i]);
		if(right >= left) {
			this.currentProblem.solution.precense[i] = 1;
			return right;
		} else {
